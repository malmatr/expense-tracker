...
export default function FinanceContextProvider({ children }) {
  ...
  const deleteExpenseItem = async (updatedExpense, expenseCategoryId) => {
    try {
      ...
      setExpenses((prevExpenses) => {
        ...
        const pos = updatedExpenses.findIndex((ex) => ex.id === expenseId);
  ...
  const deleteExpenseCategory = async (expenseCateogryId) => {
    try {
      const docRef = doc(db, "expenses", expenseCateogryId);
      await deleteDoc(docRef);

      setExpenses((prevExpenses) => {
        const updatedExpenses = prevExpenses.filter(
          (expense) => expense.id !== expenseCateogryId,
        );
